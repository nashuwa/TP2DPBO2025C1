...
void addProduk(int id, string name, string kategori, int harga, int stok, string jenis, string bahan, string warna, string untuk, string size, string merk, bool input = false)
{   
...
void showProduk()
{
    ...
    size_t max_id = 2, max_name = 4, max_kategori = 8, max_harga = 5, max_stok = 4;
    ...
    for (const auto &produk : daftarproduk)
    {
        ...
        max_stok = max(max_stok, to_string(produk.get_stok()).size());
    ...
    for (const auto &produk : daftarproduk)
    {
        cout << "| " << setw(max_id) << left << produk.get_id() << " | " 
            ...
            << setw(max_stok) << left << produk.get_stok() << " | "
    ...
    cout << "+" << string(max_id + 2, '-') << "+" << string(max_name + 2, '-') 
        << "+" << string(max_kategori + 2, '-') << "+" << string(max_harga + 2, '-')
        << "+" << string(max_stok + 2, '-')  // Tambahkan kolom stok di bawah
        << "+" << string(max_jenis + 2, '-') << "+" << string(max_bahan + 2, '-')
        << "+" << string(max_warna + 2, '-') << "+" << string(max_untuk + 2, '-')
        << "+" << string(max_size + 2, '-') << "+" << string(max_merk + 2, '-') << "+\n";
}
...
int main()
{   
    // Perbaikan: menambahkan nilai stok yang sesuai (integer)
    addProduk(1, "hohoho", "Aksesoris", 10000, 5, "Baju", "Polyester", "Merah", "Kucing", "L", "yoy");
    addProduk(2, "hahaha", "Aksesoris", 20000, 10, "Baju", "Katun", "Pink", "Anjing", "M", "yay");
    addProduk(3, "hihihi", "Aksesoris", 30000, 15, "Baju", "Sutra", "Biru", "Hamster", "S", "yiy");
    addProduk(4, "hehehe", "Aksesoris", 40000, 20, "Baju", "Katun jepang", "Ungu", "Ikan", "XS", "yey");
    addProduk(5, "huhuhu", "Aksesoris", 500000, 25, "Baju", "Wol", "Hijau", "Domba", "XL", "yuy");
    ...
    do
    {
        ...
        switch (pilihan)
        {
            ...
            case 2:
            {
                int id, harga, stok;  // Tambahkan variabel stok
