...
def addProduk(id, name, kategori, harga, jenis, bahan, warna, untuk, size, merk, input_user=False):
    global daftarproduk
    # Membuat objek Baju
    produk = Baju()
...
def showProduk():
    ...
    for produk in daftarproduk:
        data = [str(produk.get_id()), produk.get_name(), produk.get_kategori(), str(produk.get_harga()), produk.get_jenis(), produk.get_bahan(), produk.get_warna(), produk.get_untuk(), produk.get_size(), produk.get_merk()]
    ...
    # Cetak isi tabel
    for produk in daftarproduk:
        row = [str(produk.get_id()), produk.get_name(), produk.get_kategori(), str(produk.get_harga()),produk.get_jenis(), produk.get_bahan(), produk.get_warna(), produk.get_untuk(), produk.get_size(), produk.get_merk()]
...
def main():
    ...
    while True:
        ...
        if pilihan == "1":
            ...
            addProduk(id, name, kategori, harga, jenis, bahan, warna, untuk, size, merk, input_user=True)
